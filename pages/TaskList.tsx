...
export const TaskList: React.FC<TaskListProps> = ({ onSelectTask, tasks, userRole = 'contributor', onDeleteTask }) => {
 ...
 const filteredTasks = tasks
  ...
  const campaigns = [
    { name: 'Project Echo Home', type: 'Voice AI', count: 12, urgency: 'High' },
    { name: 'Project Autonomy', type: 'Computer Vision', count: 8, urgency: 'Medium' },
    { name: 'Project Indic Voice', type: 'Translation', count: 24, urgency: 'High' }
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* Featured Campaigns - Hide for admin to focus on list, or show as overview */}
      {userRole === 'contributor' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {campaigns.map((camp, idx) => (
            <div key={idx} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-5 rounded-xl shadow-sm relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-2 opacity-50">
                  <Briefcase className="h-12 w-12 text-zinc-100 dark:text-zinc-800" />
                </div>
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-2">
                      <span className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">{camp.type}</span>
                      {camp.urgency === 'High' && <span className="flex h-2 w-2 rounded-full bg-red-500 animate-pulse"></span>}
                  </div>
                  <h3 className="font-bold text-lg text-zinc-900 dark:text-white mb-1">{camp.name}</h3>
                  <p className="text-sm text-zinc-500">{camp.count} Tasks Available</p>
                  <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-white/5 flex items-center text-xs text-zinc-400 gap-1">
                      <Zap className="h-3 w-3" /> Priority Campaign
                  </div>
                </div>
            </div>
          ))}
